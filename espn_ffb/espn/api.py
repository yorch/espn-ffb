...
def get_league_years(config: Type[Config]) -> Sequence[int]:
    ...
    """
    league_office_url = "https://fantasy.espn.com/apis/v3/games/FFL/seasons/{current_year}/segments/0/leagues/" \
                        "{league_id}?view=mSettings"
    data = requests.get(
        league_office_url.format(current_year=config.CURRENT_YEAR, league_id=config.LEAGUE_ID),
